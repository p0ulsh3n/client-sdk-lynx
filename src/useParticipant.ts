...
export function useParticipant(participant: Participant): ParticipantState {
  ...
  useEffect(() => {
    const onPublicationsChanged = () => {
      ...
      setSubscribedTracks(
        Array.from(participant.trackPublications.values()).filter(
          (pub) => pub.isSubscribed && pub.track !== undefined,
        ),
      );
    };

    const onMuted = (pub: TrackPublication) => {
      if (pub.kind === Track.Kind.Audio) setAudioMuted(true);
      else if (pub.kind === Track.Kind.Video) setVideoMuted(true);
    };

    const onUnmuted = (pub: TrackPublication) => {
      if (pub.kind === Track.Kind.Audio) setAudioMuted(false);
      else if (pub.kind === Track.Kind.Video) setVideoMuted(false);
    };

    const onMetadataChanged = () => {
      if (participant.metadata) setMetadata(participant.metadata);
    };

    const onSpeakingChanged = () => setSpeaking(participant.isSpeaking);
    const onQualityUpdate   = () => setConnectionQuality(participant.connectionQuality);
    ...
    participant
      ...
      .on(ParticipantEvent.IsSpeakingChanged,           onSpeakingChanged)
      ...
      .on(ParticipantEvent.ConnectionQualityChanged,    onQualityUpdate);
    ...
    return () => {
      participant
        ...
        .off(ParticipantEvent.IsSpeakingChanged,           onSpeakingChanged)
        ...
        .off(ParticipantEvent.ConnectionQualityChanged,    onQualityUpdate);
    };
  }, [participant]);
  ...
  if (isAudioMuted !== muted) setAudioMuted(muted);
